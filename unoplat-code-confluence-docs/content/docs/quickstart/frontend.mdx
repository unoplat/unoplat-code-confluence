...
description: "Run Unoplat Code Confluence through the browser"
---
...
<Callout type="info" title="Prefer the CLI + agent?">
This page covers the **browser/Web-UI** workflow. If you'd rather drive the whole lifecycle from a coding agent or the terminal, see the [CLI + Agent Skill Quick Start](/docs/quickstart/how-to-run). Both paths talk to the same backend — pick whichever fits your workflow.
</Callout>
...
| Tool | Purpose |
|------|---------|
| [**Docker & Docker Compose**](https://docs.docker.com/get-docker/) | For running services |
...
- When prompted, select your repository provider and enter a Personal Access Token (PAT) with at least the `repo` scope for the repositories you want to ingest.
