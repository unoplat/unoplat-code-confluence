...
import { Settings, Brain, Plug, Package, GitBranch, Target, Layers, Shield, Zap } from 'lucide-react';
...
<Cards>
  <Card icon={<Settings />} title="Engineering Workflow">
    Canonical **install/build/dev/test/lint/type_check** commands and key **config files** with clear purpose
  </Card>
...
<Cards>
  <Card icon={<Target />} title="Precision First">
    **Extensible Language Support**: Modular **Tree-sitter** based grammar extraction delivers *consistent, accurate* code context across all programming languages.
    ...
    [Extensible Framework-Aware Parsing](/docs/contribution/custom-framework-schema): Specialized grammar engines recognize *framework and library-specific patterns*.
  </Card>
