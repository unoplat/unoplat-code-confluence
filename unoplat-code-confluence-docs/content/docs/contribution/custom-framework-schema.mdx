...
description: "Define framework and library patterns for Code Confluence"
---
...
<Callout type="info" title="Quick Reference">
  - **Schema**: [View Full Schema](/schemas/custom-framework-lib-schema.json) *(opens as JSON)*
...
<TypeTable
  type={{
    ...
    locator_strategy: {
      description: 'Strategy for locating the feature in code',
      type: '"VariableBound" | "Direct"',
    },
  }}
/>
...
<TypeTable
  type={{
    VariableBound: {
      description: 'First discover identifiers bound to a constructor/factory, then find their usages. Use when the symbol is assigned to a variable before use.',
      type: 'enum',
    },
    Direct: {
      description: 'Match the symbol verbatim in code. Use when the symbol is used directly without variable assignment.',
      type: 'enum',
    },
  }}
/>

<Callout type="info" title="Strategy Examples">
  ...
  **Direct** — When the import is used directly:
  ```python
  from pydantic import BaseModel
  class User(BaseModel): ...  # Direct match on 'BaseModel'
  ```
</Callout>
...
<Accordions>
  <Accordion title="View Complete Multi-Framework Schema" id="complete-example">
    Here's how **multiple frameworks** can be combined in a single schema file. This example shows a typical Python project using FastAPI for APIs, Pydantic for validation, and SQLAlchemy for database access:
...
<Accordions>
  ...
  <Accordion title="Feature Not Detected" id="error-not-detected">
    ...
    **Possible Causes:**
    1. **Wrong `locator_strategy`** — Try switching between `VariableBound` and `Direct`
    2. **Incorrect `absolute_paths`** — Verify the exact import path used in the library
