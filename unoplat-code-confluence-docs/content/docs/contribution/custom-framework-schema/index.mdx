...
<Callout type="info" title="Quick Reference">
  - **Schema (v4.1)**: [custom-framework-lib-schema-v4.1.json](/schemas/custom-framework-lib-schema-v4.1.json) *(opens as JSON)*
  - **Latest alias**: [custom-framework-lib-schema.json](/schemas/custom-framework-lib-schema.json) *(currently points to v4.1)*
...
The following capability families are taken from [`custom-framework-lib-schema-v4.1.json`](/schemas/custom-framework-lib-schema-v4.1.json). Only these values are accepted as capability keys.
...
<Accordions>
  ...
  |----------|---------------|
  | **App interfaces / entrypoints** | `rest_api`, `graphql_api`, `rpc_server`, `websocket_server`, `sse_server`, `webhook_receiver`, `webhook_sender`, `cli_command`, `scheduler`, `background_worker`, `file_watcher`, `mqtt_server`, `tcp_server`, `udp_server` |
  | **Clients / integrations** | `http_client`, `graphql_client`, `rpc_client`, `websocket_client`, `email_service`, `message_broker`, `job_queue`, `change_data_capture` |
  | **Bidirectional integrations** | `realtime_sync` |
...
<Callout type="info" title="How capabilities feed architecture">
  Framework operations classified by this enum are normalized into **inbound**, **outbound**, **bidirectional**, or **internal** interface constructs. `app_interfaces.md` preserves those constructs with source anchors, and repository architecture generation starts from the fresh interface artifacts produced by the current run.

  Capability families and downstream interface kinds are related taxonomies, but they are intentionally not a one-to-one list: a capability or one of its operations can map to a normalized architectural role.
</Callout>
...
<Callout type="info" title="CallExpression Matching Policies">
  Omit `match_policy` to use the provenance-aware `match_callee` default. Use `import_guarded_regex` only for ambiguous instance-receiver APIs that exact callee matching cannot represent. It is restricted to `CallExpression`, requires a non-empty `callee_regex`, and produces candidates only after an `absolute_paths` import gate succeeds. Because it does not prove receiver provenance, pair it with `base_confidence < 0.70` and concrete validation guidance in `notes`.
</Callout>
...
<TypeTable
  type={{
    ...
    match_policy: {
      description: 'CallExpression matching policy. match_callee requires exact import-bound callee provenance. import_guarded_regex generates regex-selected candidates after the absolute-path import gate and leaves receiver provenance to downstream validation.',
      type: 'enum: match_callee | import_guarded_regex',
      default: 'match_callee',
    },
    superclass_regex: {
...
This example combines an inheritance-based model operation with low-confidence candidate generation for SQL writes performed through runtime `Session` instances.
      ...
      "capabilities": {
        "relational_database": {
          ...
          "operations": {
            ...
            "db_sql": {
              "description": "SQLAlchemy insert construction and Session persistence calls.",
              "absolute_paths": [
                "sqlalchemy.insert",
              ...
              "construct_query": {
                ...
                "callee_regex": "^(?:insert[A-Za-z0-9_]*|[A-Za-z_][A-Za-z0-9_]*insert[A-Za-z0-9_]*|(?:[A-Za-z_][A-Za-z0-9_]*\\.)+(?:execute|add|add_all|insert))$"
              },
              "base_confidence": 0.49,
              "notes": "Trace the receiver to a SQLAlchemy Session, confirm a relational database write, and reject generic-name collisions or read-only execute calls."
            }
          }
        }
      }
...
<Callout type="warn" title="Mandatory Constraints">
  ...
  - `construct_query.match_policy` is supported only for **CallExpression** operations; omit it to use the `match_callee` default
  - `match_policy: "import_guarded_regex"` requires a non-empty `callee_regex` and should be used only for import-gated candidate generation that downstream validation can disambiguate
