...
class CodeConfluencePackageModule(dspy.Module):
    ...
    def forward(self, class_objective_list: List[DspyUnoplatNodeSummary],package_name: str):
        ...
        for class_objective in class_objective_list:
            signature_package_summary: CodeConfluencePackageSignature = self.generate_package_summary(package_existing_summary=package_summary, class_objective=class_objective.node_objective,hint="Generate the package summary for the package by being concise , factual and grounded for:"+package_name)
            package_summary = signature_package_summary.final_package_summary
        
            
        class_objective_signature: CodeConfluencePackageObjectiveSignature = self.generate_package_objective(final_package_summary=package_summary,hint="Generate the package objective for the package by being concise and dnt miss on any details for:"+package_name)
