...
class PythonImportCommentParser:
    ...
    SECTION_PATTERNS = {
        ImportType.STANDARD: [r"#\s*Standard\s+Library\s*"],
        ImportType.EXTERNAL: [r"#\s*Third\s+Party\s*"],
        ImportType.INTERNAL: [r"#\s*First\s+Party\s*"],
        ImportType.LOCAL: [r"#\s*Local\s*"]
    }
    
    def __init__(self):
        # Compile all patterns for better performance
        self.compiled_patterns = {
            import_type: [re.compile(pattern, re.IGNORECASE) 
                         for pattern in patterns]
            for import_type, patterns in self.SECTION_PATTERNS.items()
        }
    ...
    def parse_import_sections(self, file_content: str) -> Dict[ImportType, List[str]]:
        ...
        """
        sections: Dict[ImportType, List[str]] = {
            ImportType.STANDARD: [],
            ImportType.EXTERNAL: [],
            ImportType.INTERNAL: [],
            ImportType.LOCAL: []
