...
status: To Do
assignee: []
created_date: '2026-03-16 10:50'
updated_date: '2026-03-17 11:03'
labels:
  - ingestion
  - frontend
  - backend
...
Give repository ingestion a fast auto-detect path with an optional review step so users can trim or adjust detected codebases before starting workflows, then treat the approved codebase config as the default source of truth for later repository operations. This closes the current gap between backend support for explicit repository metadata and the frontend’s always-auto-detect flow, while fixing TypeScript workspace detection so monorepos ingest the intended leaf packages instead of collapsing to a root-only result.
...
<!-- SECTION:PLAN:BEGIN -->
Implementation plan (updated for alpha-only rollout)

Design source reviewed on Paper page `codebase-configuration-flow` with 7 product screens plus the onboarding baseline artboard. This work should follow the designed alpha flow directly and does not need to preserve legacy ingestion compatibility beyond what is useful during implementation.
...
- `unoplat-code-confluence-ingestion/code-confluence-flow-bridge/src/code_confluence_flow_bridge/main.py:1075` already accepts explicit `repository_metadata`, and `main.py:1102` only auto-detects when metadata is missing or empty.
...
Product direction updated: this feature is shipping in alpha, so the task no longer assumes backward compatibility is required for legacy null-metadata ingestion paths.
