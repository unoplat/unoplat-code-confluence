...
- Screen 6 (`Codebase Configuration`) is read-oriented and shows the persisted saved config as the source of truth for repository operations.
- Screen 7 (`Re-detect Codebases?`) makes replacement explicit: re-detect does not silently overwrite saved config; it routes users into a reviewed replacement flow.
...
- Because `main.py` now only owns `/start-ingestion` after TASK-16.3/16.4/16.5, TASK-16.6 is a good place to extract ingestion-specific routes into a dedicated router/service pair so detection preview, create ingestion, refresh validation, and reviewed replacement semantics live together.
...
Deferred for now on 2026-03-19. Keep in backlog and do not start implementation until reprioritized.

Superseded by standalone TASK-17 on 2026-03-19 so TASK-16 can complete without carrying this deferred alpha backend follow-up.
<!-- SECTION:NOTES:END -->
