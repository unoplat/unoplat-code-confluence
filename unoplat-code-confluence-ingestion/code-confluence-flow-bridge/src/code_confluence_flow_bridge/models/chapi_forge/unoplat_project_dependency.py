...
class UnoplatProjectDependency(BaseModel):
    version: UnoplatVersion = Field(
        default_factory=UnoplatVersion,
    ...
    extras: Optional[List[str]] = Field(
        default=None,
        description="List of extras for this dependency"
    )
    source: Optional[str] = Field(
        default=None,
        description="Source of the dependency (e.g. 'git', 'url', 'path')"
    )
    source_url: Optional[str] = Field(
        default=None,
        description="URL or path to the dependency source"
    )
