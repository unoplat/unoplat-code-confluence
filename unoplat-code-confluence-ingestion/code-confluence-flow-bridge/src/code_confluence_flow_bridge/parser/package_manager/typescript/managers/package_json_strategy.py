...
class ParsedDependencySpec(TypedDict):
    """Structured dependency-spec payload returned by package.json parsing helpers."""
...
class PackageJsonMetadataFields(TypedDict):
...
class TypeScriptPackageJsonStrategy(PackageManagerStrategy):
    """Strategy for TypeScript package managers backed by `package.json` manifests.

    This class handles the shared package.json ecosystem used by npm, pnpm, yarn,
    and bun within the canonical TypeScript namespace.
    """

    def process_metadata(
        self, local_workspace_path: str, metadata: ProgrammingLanguageMetadata
    ) -> UnoplatPackageManagerMetadata:
        """Process package.json metadata for TypeScript projects.
        ...
        logger.info(
            "Processing TypeScript package.json manifest",
            extra={
    ...
    def _collect_metadata_fields(
        ...
        if manifest.author:
            if isinstance(manifest.author, str):
                ...
                author_mapping = self._coerce_string_key_mapping(manifest.author)
                name = self._coerce_string(author_mapping.get("name"))
                email = self._coerce_string(author_mapping.get("email"))
                if name and email:
        ...
        for maintainer in manifest.maintainers:
            if isinstance(maintainer, str):
                ...
                maintainer_mapping = self._coerce_string_key_mapping(maintainer)
                name = self._coerce_string(maintainer_mapping.get("name"))
                email = self._coerce_string(maintainer_mapping.get("email"))
                if name and email:
        ...
        if manifest.repository:
            if isinstance(manifest.repository, str):
                ...
                repository_mapping = self._coerce_string_key_mapping(
                    manifest.repository
                )
                repository_url = self._coerce_string(repository_mapping.get("url"))
        ...
        return {
            ...
            "license": self._normalize_license(manifest.license),
            "readme": self._coerce_string(manifest.readme),
    ...
    def _infer_typescript_version(
        ...
        """Infer the TypeScript version or fall back to the Node engine range.
        ...
        engines = self._coerce_string_key_mapping(manifest.engines)
        node_engine = self._coerce_string(engines.get("node"))
        if node_engine:
            return node_engine.strip()

        return None

    def _flatten_exports(self, exports_node: object) -> dict[str, str]:
        ...
        stack: list[tuple[str, object]] = [("export", exports_node)]
        ...
        while stack:
            ...
            if isinstance(node, str):
                ...
                node_mapping = self._coerce_string_key_mapping(node)
                if not node_mapping:
                    continue
                # Push children onto stack in reverse order to maintain key order
                for key in reversed(list(node_mapping.keys())):
                    value = node_mapping[key]
    ...
    def _build_entry_points(self, manifest: PackageJsonManifest) -> dict[str, str]:
        ...
        if manifest.exports:
            exports_mapping = self._coerce_string_key_mapping(manifest.exports)
            if exports_mapping:
                flattened = self._flatten_exports(exports_mapping)
    ...
    def _parse_git_spec(self, spec: str) -> ParsedDependencySpec:
    ...
    def _extract_override_version(self, version_spec: str) -> str | None:
        ...
        if version_spec.startswith("{") or version_spec.startswith("["):
            try:
                ...
                parsed_mapping = self._coerce_string_key_mapping(parsed)
                version = self._coerce_string(parsed_mapping.get("version"))
    ...
    @staticmethod
    def _coerce_string_key_mapping(value: object) -> dict[str, object]:
    ...
    def _normalize_license(self, value: object) -> dict[str, object] | None:
        ...
        license_mapping = self._coerce_string_key_mapping(value)
