...
from src.code_confluence_flow_bridge.models.detection.python.rules import (
    RuleManagerConfig,
    RulesFileConfig,
    RuleSignatureConfig,
)
...
async def load_python_language_rules(rules_path: str) -> LanguageRules:
    """Load and validate Python package-manager rules from YAML."""
    ...
    return LanguageRules(
        ignores=rules_config.python.ignores,
        managers=[
            _build_manager_rule(manager_config)
            for manager_config in rules_config.python.managers
        ],
    )
