...
class ParsedRequirementLine(TypedDict, total=False):
    type: Literal["requirement", "include", "editable", "vcs"]
    requirement: Requirement | None
    path: str | None
    vcs_info: tuple[str, UnoplatProjectDependency]
    name: str
...
class PipStrategy(PackageManagerStrategy):
                ...
                for req_file in requirement_files:
                    try:
                        ...
                        for parsed in parsed_lines:
                            ...
                            if parsed_type == "requirement":
                            ...
                            elif parsed_type == "editable":
                                vcs_info = parsed.get("vcs_info")
                                editable_requirement = parsed.get("requirement")
                                ...
                                elif editable_requirement is not None:
                                    self._add_dependency_to_group(
                                        package_metadata.dependencies,
                                        group_name,
                            ...
                            elif parsed_type == "vcs":
                                # Handle VCS dependency
                                name = parsed.get("name")
                                dep = parsed.get("dependency")
                                if name is None or dep is None:
                                    continue
                                bucket = self._ensure_group(
                                    package_metadata.dependencies, group_name
                                )
    ...
    def _parse_vcs_line(self, line: str) -> tuple[str, UnoplatProjectDependency] | None:
        """
        ...
        try:
            ...
            logger.warning("Error parsing VCS line '{}': {}", line, str(e))
            return None
    ...
    def _parse_requirement_line(self, line: str) -> ParsedRequirementLine | None:
    ...
    def _parse_requirement_file_recursive(
        ...
        try:
            ...
            logger.error("Error parsing requirements file {}: {}", abs_path, str(e))
    ...
    def _add_dependency_to_group(
        ...
        if is_editable:
            logger.debug("Editable install: {}", requirement.name)
