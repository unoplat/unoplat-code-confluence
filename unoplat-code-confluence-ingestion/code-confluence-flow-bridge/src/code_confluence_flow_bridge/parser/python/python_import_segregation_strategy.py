...
class PythonImportSegregationStrategy:
    ...
    def process_imports(self, class_metadata: ChapiNode) -> Dict[ImportType, List[UnoplatImport]]:
        ...
        import_sections: Dict[ImportType, List[str]] = self.comment_parser.parse_import_sections(file_content)
                ...
                for import_str in imports:
                    ...
                    if parts[0] == "from":
                        ...
                        for name in import_names:
                            ...
                            if len(name_parts) > 1:
                                # If there's an alias
                                imported_names.append(ImportedName(original_name=name_parts[0].strip(), alias=name_parts[1].strip().rstrip(",")))
                            else:
                                # If no alias
                                imported_names.append(ImportedName(original_name=name_parts[0].strip()))
                    else:
                        # Handle 'import module as m1, module2 as m2' case
                        imports_list.extend(self._process_simple_imports(parts, section_type))

                    if parts[0] == "from":
                        imports_list.append(UnoplatImport(Source=source, UsageName=imported_names, ImportType=section_type))
        ...
        for part in parts[1:]:  # Skip 'import'
            ...
            if part.endswith(","):
                ...
                if len(current_module) > 1:
                    # Has alias
                    result.append(UnoplatImport(Source=current_module[0], UsageName=[ImportedName(original_name=current_module[0], alias=current_module[1])], ImportType=section_type))
                else:
                    # No alias
                    result.append(UnoplatImport(Source=current_module[0], UsageName=[ImportedName(original_name=current_module[0])], ImportType=section_type))
                current_module = []
            else:
