...
class SimplifiedPythonDetector:
    ...
    def _collect_nested_functions_recursively(self, func: FunctionInfo) -> List[FunctionInfo]:
        ...
        for nested_func in func.nested_functions:
            ...
            nested_functions.extend(self._collect_nested_functions_recursively(nested_func))
    ...
    def _extract_decorators_from_function(
        ...
        for match in re.finditer(decorator_pattern, func.signature, re.MULTILINE):
            ...
            detection = AnnotationLikeInfo(
                ...
                metadata={"concept": "AnnotationLike", "source": "structural_signature"}
            )
            ...
            for call in func.function_calls:
                ...
                if self._matches_call_pattern(call, spec, aliases):
                    detection = CallExpressionInfo(
                        ...
                        metadata={"concept": "CallExpression", "source": "structural_signature"}
                    )
                    detections.append(detection)
        
        return detections
    
    def _matches_call_pattern(self, call: str, spec: FeatureSpec, aliases: Dict[str, str]) -> bool:
            ...
            for base_class in base_classes:
                if self._matches_inheritance_pattern(base_class, spec, aliases):
                    ...
                    detection = InheritanceInfo(
                        ...
                        metadata={"concept": "Inheritance", "source": "structural_signature"}
                    )
                    detections.append(detection)
    ...
    def _matches_inheritance_pattern(self, base_class: str, spec: FeatureSpec, aliases: Dict[str, str]) -> bool:
    ...
    def _is_constructor_assignment(self, var_signature: str, spec: FeatureSpec, aliases: Dict[str, str]) -> bool:
            ...
            if method.signature.strip().startswith("def __init__"):
                ...
                for call in method.function_calls:
                    if self._is_constructor_call(call, spec, aliases):
                        ...
                        var_name = self._infer_instance_variable_name(call, spec, aliases)
                        if var_name:
                            instance_vars.add(var_name)
        
        return instance_vars
    
    def _is_constructor_call(self, call: str, spec: FeatureSpec, aliases: Dict[str, str]) -> bool:
        """
    ...
    def _infer_instance_variable_name(self, call: str, spec: FeatureSpec, aliases: Dict[str, str]) -> Optional[str]:
        """
        ...
        for abs_path in spec.absolute_paths:
            ...
            if func_name.endswith(f".{constructor_name}") and constructor_name in naming_conventions:
                return naming_conventions[constructor_name]
            ...
            if len(module_parts) >= 2:
                ...
                if module_name in aliases:
                    module_alias = aliases[module_name]
                    expected_call = f"{module_alias}.{constructor_name}"
                    if func_name == expected_call and constructor_name in naming_conventions:
                        return naming_conventions[constructor_name]
        
        # Fallback: use lowercase constructor name
        for abs_path in spec.absolute_paths:
            constructor_name = abs_path.split('.')[-1]
            if func_name == constructor_name or func_name.endswith(f".{constructor_name}"):
