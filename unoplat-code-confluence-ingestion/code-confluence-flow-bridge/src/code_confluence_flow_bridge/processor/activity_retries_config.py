...
class ActivityRetriesConfig:
    ...
    DEFAULT_INITIAL_INTERVAL_S = float(
        os.getenv("ACTIVITY_RETRY_INITIAL_INTERVAL_S", "3")
    )
    DEFAULT_BACKOFF_COEFFICIENT = float(
        os.getenv("ACTIVITY_RETRY_BACKOFF_COEFFICIENT", "2.0")
    )
    ...
    def for_quick_retries(attempts: Optional[int] = None) -> RetryPolicy:
        ...
        max_attempts = (
            ...
            else int(os.getenv("ACTIVITY_QUICK_RETRIES_ATTEMPTS", "5"))
        )
        initial_interval_ms = float(
            os.getenv("ACTIVITY_QUICK_INITIAL_INTERVAL_MS", "100")
        )
        backoff_coefficient = float(
            os.getenv("ACTIVITY_QUICK_BACKOFF_COEFFICIENT", "1.5")
        )
