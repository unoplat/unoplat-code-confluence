...
from code_confluence_flow_bridge.processor.db.postgres.custom_grammar_metadata import (
    Framework, FrameworkFeature, FeatureAbsolutePath
)
...
async def get_all_framework_features_for_language(
    ...
    try:
        ...
        logger.debug(f"Found {len(feature_specs)} total framework features for language={language}")
        
        return feature_specs
        
    except Exception as e:
        logger.error(f"Failed to query all framework features for language {language}: {e}")
        return []