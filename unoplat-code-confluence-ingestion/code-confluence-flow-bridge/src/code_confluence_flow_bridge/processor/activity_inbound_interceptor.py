...
class ActivityStatusInboundInterceptor(ActivityInboundInterceptor):
    ...
    async def execute_activity(self, input: ExecuteActivityInput) -> Any:
        ...
        if activity_type in [
            "update-repository-workflow-status",
            "update-codebase-workflow-status",
        ]:
        ...
        try:
            ...
            if "repository_owner_name" in input.headers:
                repository_owner_name = input.headers[
                    "repository_owner_name"
                ].data.decode("utf-8")

            log.debug(
                f"Extracted repository name from headers inside activity interceptor: {repository_name}"
            )
            log.debug(
                f"Extracted repository owner name from headers inside activity interceptor: {repository_owner_name}"
            )
            ...
            if "parent_workflow_run_id" in input.headers:
                ...
                parent_workflow_run_id = parent_workflow_run_id_payload.data.decode(
                    "utf-8"
                )
                log.debug(
                    f"Extracted parent_workflow_run_id from headers: {parent_workflow_run_id}"
                )
            ...
            if "codebases" in input.headers:
                ...
                log.debug(
                    f"Extracted codebase configs from headers: {codebase_configs}"
                )
        except Exception as e:
            # We'll set up a basic logger below
            log.error(
                f"Failed to extract contextual information from headers: {str(e)}"
            )
            raise e
        ...
        log.debug(
            f"Activity context: trace_id={trace_id}, repo={repository_name}/{repository_owner_name}, "
        ...
        if not (trace_id and repository_name and repository_owner_name):
            log.warning(
                "Missing essential context from headers. Status updates may be incomplete."
            )
        ...
        log.debug(
            f"Skipping RUNNING state update for activity {activity_type} as it's now handled by the workflow interceptor"
        )
            ...
            try:
                ...
                if workflow_type == "repo-activity-workflow":
                    ...
                    if (
                        ...
                        await self.parent_db_activity.update_repository_workflow_status(
                            parent_env
                        )
                ...
                # Handle child workflow (child-codebase-workflow) status updates
                    ...
                    try:
                        if "codebase_folder" in input.headers:
                            target_codebase_folder = input.headers[
                                "codebase_folder"
                            ].data.decode("utf-8")
                            log.debug(
                            ...
                            log.warning(
                                "codebase_folder not found in headers, cannot update child workflow status"
                            )
                    ...
                    if target_codebase_folder:
                        try:
                            ...
                            await (
                                self.child_db_activity.update_codebase_workflow_status(
                                    child_env
                                )
                            )
                            ...
                            log.error(
                                f"Failed to update {status_to_mark.value} status: {status_update_error}"
                            )
            except Exception as outer_error:
                log.error(
                    f"Error in activity interceptor error handling: {outer_error}"
                )
