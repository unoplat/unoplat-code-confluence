...
'''
    ...
    extractor: TreeSitterPythonStructuralSignatureExtractor = (
        TreeSitterPythonStructuralSignatureExtractor(
    ...
    foo_class = next(
        cl for cl in signature.classes if cl.signature.startswith("class Foo")
    )
    ...
    # Verify call counts
    inner_calls = [
        call for call in outer_function.function_calls if call.startswith("inner(")
    ]
    assert len(inner_calls) == 2
    print_calls = [
        call for call in outer_function.function_calls if call.startswith("print(")
    ]
    ...
    inner_function = next(
        nf
        for nf in outer_function.nested_functions
        if nf.signature.startswith("def inner")
    )
    ...
    if signature.classes:
        ...
        total_function_calls = sum(
            len(func.function_calls) for func in signature.functions
        ) + sum(
            len(method.function_calls)
...
@pytest.mark.parametrize("language_name", ["python"])
def test_structural_signature_extraction_complex(
    tmp_path: Path, language_name: str
) -> None:
...
'''
    ...
    assert any(
        sig_line.startswith("@classmethod") or sig_line.startswith("def greet")
        for sig_line in method_sigs
    )
    assert any(
        sig_line.startswith("async def age_next_year") for sig_line in method_sigs
    )
    ...
    fetch_data_fn = next(
        fn for fn in sig.functions if "async def fetch_data" in fn.signature
    )
    ...
    process_data_fn = next(
        fn for fn in sig.functions if "def process_data" in fn.signature
    )
    ...
    decorator_fn = next(
        fn for fn in sig.functions if "def simple_decorator" in fn.signature
    )
    assert len(decorator_fn.function_calls) == 0

    # The wrapper function should have exactly 1 call: fn(*args, **kwargs)
    wrapper_fn = next(
        nf for nf in decorator_fn.nested_functions if "def wrapper" in nf.signature
    )
    ...
    age_method = next(
        m for m in person_cls.methods if "async def age_next_year" in m.signature
    )
    ...
    if sig.classes:
        ...
        total_function_calls = sum(
            len(func.function_calls) for func in sig.functions
        ) + sum(
            len(method.function_calls) for cls in sig.classes for method in cls.methods
        )
        print(f"Total methods across all classes: {total_methods}")
        print(f"Total function calls: {total_function_calls}")
...
'''
    ...
    complex_func = next(
        fn for fn in sig.functions if "def complex_function" in fn.signature
    )

    # Verify no duplicate nested functions
    assert len(complex_func.nested_functions) == 1, (
        "complex_function should only have level1_nested as immediate child"
    )
    assert complex_func.nested_functions[0].signature == "def level1_nested(x):"

    # Ensure grandchildren don't appear in grandparent
    assert not any(
        "level2_nested" in nf.signature for nf in complex_func.nested_functions
    )
    assert not any(
        "level3_nested" in nf.signature for nf in complex_func.nested_functions
    )
    ...
    level1 = next(
        nf
        for nf in complex_func.nested_functions
        if "def level1_nested" in nf.signature
    )
    ...
    level2 = next(
        nf for nf in level1.nested_functions if "def level2_nested" in nf.signature
    )
    ...
    level3 = next(
        nf for nf in level2.nested_functions if "def level3_nested" in nf.signature
    )
    ...
    complex_class = next(
        cl for cl in sig.classes if "class ComplexClass" in cl.signature
    )
    method = next(
        m
        for m in complex_class.methods
        if "def method_with_comprehensions" in m.signature
    )
    ...
    helper_func = next(
        nf for nf in method.nested_functions if "def helper" in nf.signature
    )
    assert "process(x)" in helper_func.function_calls
    assert (
        "process(x)" not in method.function_calls
    )  # This is inside helper, not direct
    ...
    if sig.classes:
        total_methods = sum(len(cls.methods) for cls in sig.classes)
        total_nested_functions = sum(
            len(func.nested_functions) for func in sig.functions
        ) + sum(
            len(method.nested_functions)
...
def test_self_extraction_tree_sitter_structural_signature(language_name: str) -> None:
    ...
    assert len(signature.global_variables) == 0, (
        "Expected no global variables after cache refactor"
    )
    ...
    assert len(main_class.vars) == 0, (
        "Python subclass should have no instance variables (set by base class)"
    )
    ...
    assert len(main_class.methods) == 20, (
        f"Expected 20 methods after refactor, got {len(main_class.methods)}"
    )
    ...
    expected_methods = [
        ...
        "_LANGUAGE_CACHE",
        "_PARSER_CACHE",
        "_QUERY_CACHE",  # Wrapper properties for test compatibility
        "extract_structural_signature",
        "_extract_module_docstring",
        "_extract_global_variables",
        "_extract_functions",
        "_extract_classes",
        "_extract_class_variables_for_node",
        "_extract_methods_for_node",
        "_extract_nested_functions_for_node",
        "_is_at_module_level",
        "_clean_string_literal",
        "_is_immediate_child_function",
        "_get_nested_function_ranges",
        "_get_nested_class_ranges",
        "_extract_function_calls_for_node",
        "_extract_instance_variables_for_method",
    ...
    # Check for key components of the signature (handles multi-line formatting)
    assert "def extract_structural_signature" in extract_method.signature
    assert "self" in extract_method.signature
    assert "source_bytes: bytes" in extract_method.signature
    assert "-> PythonStructuralSignature:" in extract_method.signature
    ...
    for method in main_class.methods:
        assert len(method.nested_functions) == 0, (
            f"Method {method.signature.split('(')[0]} should have no nested functions"
        )
    ...
    print(
        f"Total function calls across all methods: {sum(len(m.function_calls) for m in main_class.methods)}"
    )
...
def test_no_duplicate_nested_functions(tmp_path: Path, language_name: str) -> None:
    ...
    def assert_unique_nested(
        func: PythonFunctionInfo, seen_signatures: Optional[set[str]] = None
    ):
        if seen_signatures is None:
            seen_signatures = set()

        for nf in func.nested_functions:
            assert nf.signature not in seen_signatures, (
                f"Duplicate found: {nf.signature}"
            )
    ...
    level1 = next(
        nf for nf in level0.nested_functions if "def level1()" in nf.signature
    )
    ...
    assert any(
        "def level2_decorated()" in nf.signature for nf in level1.nested_functions
    )
    ...
    assert not any(
        "method_deep_nested" in nf.signature for nf in method.nested_functions
    )
...
def test_main_py_structural_signature(language_name: str) -> None:
    ...
    assert (
        signature.module_docstring is None or signature.module_docstring.strip() == ""
    )
    ...
    global_var_signatures = [var.signature for var in signature.global_variables]
    assert any("logger" in sig for sig in global_var_signatures), (
        f"logger not found in {global_var_signatures}"
    )
    assert any("app" in sig for sig in global_var_signatures), (
        f"app not found in {global_var_signatures}"
    )
    assert any("origins" in sig for sig in global_var_signatures), (
        f"origins not found in {global_var_signatures}"
    )

    # 3. Functions verification
    # Should have many functions (utility, endpoints, etc.)
    assert len(signature.functions) >= 20, (
        f"Expected at least 20 functions, got {len(signature.functions)}"
    )

    function_signatures = [fn.signature for fn in signature.functions]

    # Key utility functions
    assert any("async def get_temporal_client" in sig for sig in function_signatures), (
        "get_temporal_client not found"
    )
    assert any("async def _serve_worker" in sig for sig in function_signatures), (
        "_serve_worker not found"
    )
    ...
    assert any("async def monitor_workflow" in sig for sig in function_signatures), (
        "monitor_workflow not found"
    )

    # FastAPI endpoints
    assert any("async def ingest_token" in sig for sig in function_signatures), (
        "ingest_token not found"
    )
    assert any("async def update_token" in sig for sig in function_signatures), (
        "update_token not found"
    )
    assert any("async def delete_token" in sig for sig in function_signatures), (
        "delete_token not found"
    )
    assert any("async def get_repos" in sig for sig in function_signatures), (
        "get_repos not found"
    )
    assert any("async def ingestion" in sig for sig in function_signatures), (
        "ingestion not found"
    )
    assert any(
        "async def get_repository_status" in sig for sig in function_signatures
    ), "get_repository_status not found"

    # Context manager function
    assert any("async def lifespan" in sig for sig in function_signatures), (
        "lifespan not found"
    )

    # 4. No classes should be defined directly in main.py
    assert len(signature.classes) == 0, (
        f"Expected no classes, got {len(signature.classes)}"
    )

    # 5. Function call verification for complex functions

    # create_worker function should have many calls
    create_worker_fn = next(
        (fn for fn in signature.functions if "def create_worker" in fn.signature), None
    )
    assert create_worker_fn is not None, "create_worker function not found"
    assert len(create_worker_fn.function_calls) >= 10, (
        f"create_worker should have many calls, got {len(create_worker_fn.function_calls)}"
    )

    # Check some specific calls in create_worker
    create_worker_calls = create_worker_fn.function_calls
    assert any("Worker(" in call for call in create_worker_calls), (
        "Worker instantiation not found"
    )
    assert any("logger.info(" in call for call in create_worker_calls), (
        "logger.info call not found"
    )

    # ingestion function should have specific calls
    ingestion_fn = next(
        (fn for fn in signature.functions if "async def ingestion" in fn.signature),
        None,
    )
    assert ingestion_fn is not None, "ingestion function not found"
    assert len(ingestion_fn.function_calls) >= 5, (
        f"ingestion should have several calls, got {len(ingestion_fn.function_calls)}"
    )

    ingestion_calls = ingestion_fn.function_calls
    assert any(
        "fetch_repository_provider_token(" in call for call in ingestion_calls
    ), "fetch_repository_provider_token call not found"
    assert any("start_workflow(" in call for call in ingestion_calls), (
        "start_workflow call not found"
    )
    assert any("asyncio.create_task(" in call for call in ingestion_calls), (
        "asyncio.create_task call not found"
    )

    # lifespan function should have many setup/teardown calls
    lifespan_fn = next(
        (fn for fn in signature.functions if "async def lifespan" in fn.signature), None
    )
    assert lifespan_fn is not None, "lifespan function not found"
    assert len(lifespan_fn.function_calls) >= 15, (
        f"lifespan should have many calls, got {len(lifespan_fn.function_calls)}"
    )

    lifespan_calls = lifespan_fn.function_calls
    assert any("get_temporal_client(" in call for call in lifespan_calls), (
        "get_temporal_client call not found"
    )
    assert any("CodeConfluenceGraph(" in call for call in lifespan_calls), (
        "CodeConfluenceGraph call not found"
    )
    assert any("create_db_and_tables(" in call for call in lifespan_calls), (
        "create_db_and_tables call not found"
    )
    ...
    assert len(get_repos_fn.function_calls) >= 8, (
        f"get_repos should have several calls, got {len(get_repos_fn.function_calls)}"
    )

    get_repos_calls = get_repos_fn.function_calls
    assert any("GQLClient(" in call for call in get_repos_calls), (
        "GQLClient call not found"
    )
    assert any("client.execute(" in call for call in get_repos_calls), (
        "client.execute call not found"
    )
    ...
    for fn in signature.functions:
        assert fn.start_line > prev_line, (
            f"Functions should be ordered by line number, {fn.signature} at line {fn.start_line} comes before {prev_line}"
        )
        prev_line = fn.start_line

    # Check some specific line number ranges
    get_temporal_client_fn = next(
        (
            fn
            for fn in signature.functions
            if "async def get_temporal_client" in fn.signature
        ),
        None,
    )
    assert get_temporal_client_fn is not None
    assert 140 < get_temporal_client_fn.start_line < 160, (
        f"get_temporal_client should be around line 147, got {get_temporal_client_fn.start_line}"
    )

    # 7. Check for nested functions in lifespan
    assert len(lifespan_fn.nested_functions) == 0, (
        "lifespan should not have nested functions directly"
    )
    ...
    total_nested_functions = sum(
        len(func.nested_functions) for func in signature.functions
    )
    print(f"Total nested functions: {total_nested_functions}")

    # Print some interesting statistics
    functions_with_calls = [
        fn for fn in signature.functions if len(fn.function_calls) > 0
    ]
    ...
    if functions_with_calls:
        ...
        max_calls_fn = next(
            fn for fn in functions_with_calls if len(fn.function_calls) == max_calls
        )
        print(
            f"Function with most calls: {max_calls_fn.signature.split('(')[0].strip().split()[-1]} ({max_calls} calls)"
        )

    # Print decorator usage
    decorated_functions = [
        fn for fn in signature.functions if fn.signature.strip().startswith("@")
    ]
    ...
    print(
        f"- {len(async_functions)} async functions for handling concurrent operations"
    )
    print(
        f"- {total_function_calls} total function calls indicating complex business logic"
    )
...
def test_instance_variable_edge_cases(tmp_path: Path, language_name: str) -> None:
    ...
    edge_cases_file_path = (
        Path(__file__).parent / "test_data" / "instance_variable_edge_cases.py"
    )
    assert edge_cases_file_path.exists(), (
        f"Edge cases file not found at {edge_cases_file_path}"
    )
    ...
    fastapi_class = next(
        cls for cls in signature.classes if "class FastAPIApp" in cls.signature
    )
    ...
    assert any("CLASS_CONSTANT" in sig for sig in class_var_signatures), (
        "CLASS_CONSTANT not captured"
    )
    assert any("shared_cache" in sig for sig in class_var_signatures), (
        "shared_cache not captured"
    )

    # Basic instance variables from __init__ (should be captured)
    assert any("self.app = FastAPI()" in sig for sig in class_var_signatures), (
        "self.app not captured"
    )
    assert any("self.config = config" in sig for sig in class_var_signatures), (
        "self.config not captured"
    )
    assert any("self.database_url:" in sig for sig in class_var_signatures), (
        "self.database_url not captured"
    )

    # Type-annotated instance variables (should be captured)
    assert any("self.clients:" in sig for sig in class_var_signatures), (
        "self.clients not captured"
    )
    assert any("self.cache:" in sig for sig in class_var_signatures), (
        "self.cache not captured"
    )
    assert any("self.metrics:" in sig for sig in class_var_signatures), (
        "self.metrics not captured"
    )

    # Complex expressions (should be captured)
    assert any("self.router = APIRouter" in sig for sig in class_var_signatures), (
        "self.router not captured"
    )
    assert any("self.middleware_stack =" in sig for sig in class_var_signatures), (
        "self.middleware_stack not captured"
    )
    ...
    assert any(
        "self.auth_handler = AuthHandler" in sig for sig in class_var_signatures
    ), "self.auth_handler (then) should be captured - inside if block"
    assert any("self.auth_handler = None" in sig for sig in class_var_signatures), (
        "self.auth_handler (else) should be captured - inside else block"
    )
    ...
    assert not any("self.settings.debug =" in sig for sig in class_var_signatures), (
        "self.settings.debug should not be captured - nested attribute"
    )
    assert not any("self.settings.timeout =" in sig for sig in class_var_signatures), (
        "self.settings.timeout should not be captured - nested attribute"
    )

    # Method call results (should be captured)
    assert any(
        "self.connection = self._create_connection()" in sig
        for sig in class_var_signatures
    ), "self.connection not captured"
    assert any("self.logger = setup_logger" in sig for sig in class_var_signatures), (
        "self.logger not captured"
    )

    # F-string assignments (should be captured)
    assert any("self.base_url = f" in sig for sig in class_var_signatures), (
        "self.base_url not captured"
    )
    assert any("self.version_info = f" in sig for sig in class_var_signatures), (
        "self.version_info not captured"
    )

    # Lambda and comprehensions (should be captured)
    assert any("self.data_processor = lambda" in sig for sig in class_var_signatures), (
        "self.data_processor not captured"
    )
    assert any("self.valid_routes =" in sig for sig in class_var_signatures), (
        "self.valid_routes not captured"
    )

    # Instance variables from configure_middleware method (should be captured)
    assert any("self.cors_middleware =" in sig for sig in class_var_signatures), (
        "self.cors_middleware not captured"
    )
    assert any("self.rate_limiter =" in sig for sig in class_var_signatures), (
        "self.rate_limiter not captured"
    )
    assert any("self.security_config =" in sig for sig in class_var_signatures), (
        "self.security_config not captured"
    )

    # Complex nested assignments (NOT captured - Tree-sitter query doesn't match deeply nested attributes)
    # This is expected behavior - our query only matches simple self.attr assignments
    assert not any(
        "self.nested.handler.config =" in sig for sig in class_var_signatures
    ), "self.nested.handler.config should not be captured - deeply nested attribute"
    assert not any(
        "self.deep.very.nested.setting =" in sig for sig in class_var_signatures
    ), "self.deep.very.nested.setting should not be captured - deeply nested attribute"

    # Instance variables from private method (should be captured)
    assert any("self.connection_pool =" in sig for sig in class_var_signatures), (
        "self.connection_pool not captured"
    )
    assert any("self.connection_helper =" in sig for sig in class_var_signatures), (
        "self.connection_helper not captured"
    )

    # Instance variables from async method (should be captured)
    assert any("self.async_client =" in sig for sig in class_var_signatures), (
        "self.async_client not captured"
    )
    assert any("self.event_loop =" in sig for sig in class_var_signatures), (
        "self.event_loop not captured"
    )
    assert any("self.async_data = await" in sig for sig in class_var_signatures), (
        "self.async_data not captured"
    )
    ...
    property_class = next(
        cls
        for cls in signature.classes
        if "class PropertyAndDescriptorClass" in cls.signature
    )
    ...
    assert not any("local_config =" in sig for sig in class_var_signatures), (
        "local_config incorrectly captured"
    )
    assert not any("middleware_list =" in sig for sig in class_var_signatures), (
        "middleware_list incorrectly captured"
    )
    assert not any("temp_handler =" in sig for sig in class_var_signatures), (
        "temp_handler incorrectly captured"
    )

    # Variables on other objects should NOT be captured
    assert not any("other_obj.config =" in sig for sig in class_var_signatures), (
        "other_obj.config incorrectly captured"
    )
    assert not any("different.setting =" in sig for sig in class_var_signatures), (
        "different.setting incorrectly captured"
    )

    # Variables in static/class methods should NOT be captured as instance vars
    assert not any("default_settings =" in sig for sig in class_var_signatures), (
        "default_settings incorrectly captured"
    )
    assert not any("temp_config =" in sig for sig in class_var_signatures), (
        "temp_config incorrectly captured"
    )
    assert not any("config_data =" in sig for sig in class_var_signatures), (
        "config_data incorrectly captured"
    )

    # Nested function variables should NOT be captured
    assert not any("local_conn =" in sig for sig in class_var_signatures), (
        "local_conn incorrectly captured"
    )
    assert not any("helper_config =" in sig for sig in class_var_signatures), (
        "helper_config incorrectly captured"
    )
    assert not any("inner_self.fake_attr =" in sig for sig in class_var_signatures), (
        "inner_self.fake_attr incorrectly captured"
    )

    # Async method local variables should NOT be captured
    assert not any("async_config =" in sig for sig in class_var_signatures), (
        "async_config incorrectly captured"
    )

    # Property local variables should NOT be captured
    assert not any("temp_calc =" in sig for sig in property_vars), (
        "temp_calc incorrectly captured"
    )
    assert not any("validation_result =" in sig for sig in property_vars), (
        "validation_result incorrectly captured"
    )
    ...
    nested_class_test = next(
        cls for cls in signature.classes if "class NestedClassTest" in cls.signature
    )
    nested_test_vars = [var.signature for var in nested_class_test.vars]

    # Parent class should have its own instance variables
    assert any("PARENT_CONSTANT =" in sig for sig in nested_test_vars), (
        "PARENT_CONSTANT not captured"
    )
    assert any("self.parent_attr =" in sig for sig in nested_test_vars), (
        "self.parent_attr not captured"
    )
    assert any("self.nested_instance =" in sig for sig in nested_test_vars), (
        "self.nested_instance not captured"
    )

    # Find the nested class (now properly nested in parent class)
    nested_class = nested_class_test.nested_classes[
        0
    ]  # NestedClass is now in nested_classes
    nested_class_vars = [var.signature for var in nested_class.vars]

    # Nested class should have its own separate instance variables
    assert any("NESTED_CONSTANT =" in sig for sig in nested_class_vars), (
        "NESTED_CONSTANT not captured"
    )
    assert any("self.nested_attr =" in sig for sig in nested_class_vars), (
        "self.nested_attr not captured"
    )
    assert any("self.config =" in sig for sig in nested_class_vars), (
        "nested self.config not captured"
    )

    # Verify nested class variables don't leak into parent
    assert not any("NESTED_CONSTANT =" in sig for sig in nested_test_vars), (
        "NESTED_CONSTANT leaked to parent"
    )
    assert not any("nested_attr =" in sig for sig in nested_test_vars), (
        "nested_attr leaked to parent"
    )

    # Verify parent class variables don't leak into nested
    assert not any("PARENT_CONSTANT =" in sig for sig in nested_class_vars), (
        "PARENT_CONSTANT leaked to nested"
    )
    assert not any("parent_attr =" in sig for sig in nested_class_vars), (
        "parent_attr leaked to nested"
    )
    ...
    typed_class = next(
        cls for cls in signature.classes if "class TypeAnnotatedClass" in cls.signature
    )
    typed_vars = [var.signature for var in typed_class.vars]

    # ClassVar annotated variables (should be captured)
    assert any("class_config:" in sig for sig in typed_vars), (
        "class_config not captured"
    )
    assert any("shared_state:" in sig for sig in typed_vars), (
        "shared_state not captured"
    )

    # Type-annotated instance variables (should be captured)
    assert any("self.typed_config:" in sig for sig in typed_vars), (
        "self.typed_config not captured"
    )
    assert any("self.optional_cache:" in sig for sig in typed_vars), (
        "self.optional_cache not captured"
    )
    assert any("self.generic_list:" in sig for sig in typed_vars), (
        "self.generic_list not captured"
    )
    assert any("self.union_type:" in sig for sig in typed_vars), (
        "self.union_type not captured"
    )
    assert any("self.complex_type:" in sig for sig in typed_vars), (
        "self.complex_type not captured"
    )
    ...
    assert not any("GLOBAL_CONFIG =" in sig for sig in all_class_vars), (
        "GLOBAL_CONFIG incorrectly captured"
    )
    assert not any("app_instance =" in sig for sig in all_class_vars), (
        "app_instance incorrectly captured"
    )
    assert not any("ANOTHER_GLOBAL =" in sig for sig in all_class_vars), (
        "ANOTHER_GLOBAL incorrectly captured"
    )
    assert not any("module_config =" in sig for sig in all_class_vars), (
        "module_config incorrectly captured"
    )

    # Module function variables should NOT be captured
    assert not any("local_var =" in sig for sig in all_class_vars), (
        "local_var incorrectly captured"
    )
    assert not any("fake_self.attribute =" in sig for sig in all_class_vars), (
        "fake_self.attribute incorrectly captured"
    )
    assert not any("nested_var =" in sig for sig in all_class_vars), (
        "nested_var incorrectly captured"
    )
    ...
    assert len(fastapi_class.methods) == len(expected_fastapi_methods), (
        f"Expected {len(expected_fastapi_methods)} methods, got {len(fastapi_class.methods)}"
    )
    ...
    for expected_method in expected_fastapi_methods:
        assert any(expected_method in sig for sig in method_signatures), (
            f"Method {expected_method} not found"
        )
    ...
    assert len(instance_vars) > len(class_vars), (
        "Should have more instance variables than class variables"
    )
    assert len(instance_vars) >= 20, (
        f"Should capture at least 20 instance variables, got {len(instance_vars)}"
    )
...
def test_local_variables_not_captured(tmp_path: Path, language_name: str) -> None:
    ...
    # Get the demo class
    demo_class = next(
        cls for cls in signature.classes if "class DemoClass" in cls.signature
    )
    ...
    assert any("CLASS_VAR" in sig for sig in captured_vars), (
        "CLASS_VAR should be captured"
    )

    # Instance variables should be captured
    assert any("self.instance_var" in sig for sig in captured_vars), (
        "self.instance_var should be captured"
    )
    assert any("self.app" in sig for sig in captured_vars), (
        "self.app should be captured"
    )
    assert any("self.method_instance_var" in sig for sig in captured_vars), (
        "self.method_instance_var should be captured"
    )
    assert any("self.outer_instance_var" in sig for sig in captured_vars), (
        "self.outer_instance_var should be captured"
    )
    assert any("self.after_nested" in sig for sig in captured_vars), (
        "self.after_nested should be captured"
    )
    ...
    assert not any("local_var" in sig for sig in captured_vars), (
        "local_var should NOT be captured"
    )
    assert not any("temp_config" in sig for sig in captured_vars), (
        "temp_config should NOT be captured"
    )
    assert not any("helper_obj" in sig for sig in captured_vars), (
        "helper_obj should NOT be captured"
    )

    # Assignments to other objects should NOT be captured
    assert not any("helper_obj.config" in sig for sig in captured_vars), (
        "helper_obj.config should NOT be captured"
    )

    # Local variables in regular method should NOT be captured
    assert not any("local_data" in sig for sig in captured_vars), (
        "local_data should NOT be captured"
    )
    assert not any("temp_result" in sig for sig in captured_vars), (
        "temp_result should NOT be captured"
    )
    assert not any("cache =" in sig for sig in captured_vars), (
        "cache should NOT be captured"
    )
    assert not any("items =" in sig for sig in captured_vars), (
        "items should NOT be captured"
    )
    assert not any("processed_items" in sig for sig in captured_vars), (
        "processed_items should NOT be captured"
    )

    # Local variables in static method should NOT be captured
    assert not any("static_local" in sig for sig in captured_vars), (
        "static_local should NOT be captured"
    )
    assert not any("config =" in sig for sig in captured_vars), (
        "config should NOT be captured"
    )

    # Local variables in method with nested function should NOT be captured
    assert not any("outer_local" in sig for sig in captured_vars), (
        "outer_local should NOT be captured"
    )

    # Variables in nested functions should NOT be captured
    assert not any("nested_local" in sig for sig in captured_vars), (
        "nested_local should NOT be captured"
    )
    assert not any("nested_config" in sig for sig in captured_vars), (
        "nested_config should NOT be captured"
    )
    assert not any("fake_self.attribute" in sig for sig in captured_vars), (
        "fake_self.attribute should NOT be captured"
    )
    ...
    global_var_signatures = [var.signature for var in signature.global_variables]
    assert any("GLOBAL_VAR" in sig for sig in global_var_signatures), (
        "GLOBAL_VAR should be in global variables"
    )
    assert not any("GLOBAL_VAR" in sig for sig in captured_vars), (
        "GLOBAL_VAR should NOT be in class variables"
    )

    # Module function variables should not appear anywhere in class variables
    assert not any("func_local" in sig for sig in captured_vars), (
        "func_local should NOT be captured"
    )
    assert not any("temp_obj" in sig for sig in captured_vars), (
        "temp_obj should NOT be captured"
    )
    ...
    assert len(instance_vars) == 5, (
        f"Expected 5 instance variables, got {len(instance_vars)}"
    )
    ...
    print(
        f"   (class variables + instance variables) while filtering out implementation"
    )
