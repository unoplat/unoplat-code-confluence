...
@pytest.fixture
def typescript_strategy() -> TypeScriptPackageJsonStrategy:
    """Create a TypeScriptPackageJsonStrategy instance."""
    return TypeScriptPackageJsonStrategy()


@pytest.fixture
def typescript_metadata() -> ProgrammingLanguageMetadata:
    """Create metadata for TypeScript package.json projects."""
...
def typescript_metadata_all_managers(
    request: pytest.FixtureRequest,
) -> ProgrammingLanguageMetadata:
    """Create metadata for all TypeScript package managers.

    Parametrized fixture that tests TypeScriptPackageJsonStrategy with:
    - NPM
    - YARN
    - PNPM
    - BUN
...
def test_library_package_metadata(
    typescript_strategy: TypeScriptPackageJsonStrategy,
    typescript_metadata: ProgrammingLanguageMetadata,
    library_package_dir: Path,
) -> None:
    """Ensure library packages surface TypeScript metadata and entry points."""
    metadata = typescript_strategy.process_metadata(
        str(library_package_dir), typescript_metadata
    )
...
def test_application_package_engine_fallback(
    ...
    metadata = typescript_strategy.process_metadata(
        str(application_package_dir), typescript_metadata
    )
...
def test_all_typescript_package_managers(
    typescript_strategy: TypeScriptPackageJsonStrategy,
    typescript_metadata_all_managers: ProgrammingLanguageMetadata,
    library_package_dir: Path,
) -> None:
    """Verify TypeScriptPackageJsonStrategy works with all TypeScript package managers.
    ...
    """
    metadata = typescript_strategy.process_metadata(
        str(library_package_dir), typescript_metadata_all_managers
    )
    ...
    expected_pm = (
        typescript_metadata_all_managers.package_manager.value
        if typescript_metadata_all_managers.package_manager
        else "unknown"
    )
