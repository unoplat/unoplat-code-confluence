...
def flatten_dependencies(
    grouped: Dict[str, Dict[str, UnoplatProjectDependency]],
...
def test_process_metadata_all_required_sections(
    ...
    package_metadata = mock_poetry_strategy.process_metadata(
        str(tmp_path), mock_metadata
    )
    ...
    assert any(
        "Test Author <test@example.com>" in author
        for author in package_metadata.authors or []
    )
    ...
    assert any(
        "Maintainer <maintainer@example.com>" in maintainer
        for maintainer in package_metadata.maintainers or []
    )
    assert package_metadata.readme == "README.md"

    # Test dependencies from all groups
    grouped_deps: Dict[str, Dict[str, UnoplatProjectDependency]] = (
...
def test_process_metadata_unoplat(
    ...
    assert any(
        "JayGhiya <ghiya6548@gmail.com>" in author
        for author in package_metadata.authors or []
    )
    assert package_metadata.programming_language == "python"
    assert package_metadata.package_manager == "poetry"
    ...
    assert (
        git_dep.source_url == "https://github.com/unoplat/unoplat-code-confluence.git"
    )
    ...
    assert package_metadata.entry_points is not None
    assert (
        package_metadata.entry_points["unoplat-code-confluence"]
        == "unoplat_code_confluence.__main__:main"
    )
