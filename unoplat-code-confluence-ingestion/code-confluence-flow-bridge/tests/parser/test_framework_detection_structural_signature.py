...
class TestFrameworkDetectionPythonStructuralSignature:
    ...
    def setup_method(self):
        ...
        self.codebase_child_workflow_path = (
            self.base_path / "processor" / "codebase_child_workflow.py"
        )
        # structural_signature.py has been moved to commons, use a different file for testing
        self.commons_base_path = (
        ...
        self.structural_signature_path = (
            self.commons_base_path / "python_structural_signature.py"
        )
    ...
    def test_fastapi_detection_from_real_main_py(self):
        ...
        assert "FastAPI" in frameworks, (
            f"FastAPI not detected in frameworks: {frameworks}"
        )
        ...
        if not has_pydantic_patterns:
            ...
            pydantic_in_imports = any(
                "pydantic" in var.signature.lower()
                for var in signature.global_variables
                if "import" in var.signature
            )
            print(f"Pydantic in imports: {pydantic_in_imports}")
        else:
            assert "Pydantic" in frameworks, (
                f"Pydantic patterns found but not detected in frameworks: {frameworks}"
            )
        ...
        assert len(endpoints) >= 1, (
            f"Expected at least 1 endpoint, found {len(endpoints)}"
        )

    def test_temporal_detection_from_real_workflow_files(self):
        """Test Temporal framework detection from actual workflow files."""
        assert self.repo_workflow_path.exists(), (
            f"Repo workflow not found at {self.repo_workflow_path}"
        )
        ...
        # Test framework detection
        frameworks = self._detect_frameworks(signature)
        assert "Temporal" in frameworks, (
            f"Temporal not detected in frameworks: {frameworks}"
        )
        ...
        assert "RepoWorkflow" in workflows, (
            f"RepoWorkflow not found in workflows: {workflows}"
        )

        # Test activity execution patterns
        assert self._has_activity_execution_patterns(signature), (
            "Activity execution patterns not found"
        )

        # Test child workflow patterns
        assert self._has_child_workflow_patterns(signature), (
            "Child workflow patterns not found"
        )

    def test_temporal_detection_from_child_workflow(self):
        """Test Temporal framework detection from child workflow file."""
        assert self.codebase_child_workflow_path.exists(), (
            f"Child workflow not found at {self.codebase_child_workflow_path}"
        )
        ...
        assert "CodebaseChildWorkflow" in workflows, (
            f"CodebaseChildWorkflow not found in workflows: {workflows}"
        )
    ...
    def test_pydantic_detection_from_real_structural_signature_py(self):
        ...
        assert self.structural_signature_path.exists(), (
            f"Structural signature not found at {self.structural_signature_path}"
        )
        ...
        frameworks = self._detect_frameworks(signature)
        assert "Pydantic" in frameworks, (
            f"Pydantic not detected in frameworks: {frameworks}"
        )

        # Test specific Pydantic patterns
        assert self._has_pydantic_patterns(signature), "Pydantic patterns not found"
        assert self._has_pydantic_field_patterns(signature), (
            "Pydantic Field patterns not found"
        )

        # Test model patterns
        models = self._extract_pydantic_models(signature)
        assert len(models) >= 1, (
            f"Expected at least 1 Pydantic model, found {len(models)}"
        )
        assert "PythonStructuralSignature" in models, (
            f"PythonStructuralSignature not found in models: {models}"
        )
    ...
    def test_advanced_pydantic_from_real_package_metadata_py(self):
        ...
        assert self.package_metadata_path.exists(), (
            f"Package metadata not found at {self.package_metadata_path}"
        )
        ...
        assert len(validators) >= 1, (
            f"Expected at least 1 validator, found {len(validators)}"
        )
        assert "validate_license" in validators, (
            f"validate_license not found in validators: {validators}"
        )
    ...
    def test_sqlalchemy_detection_from_real_repository_data_py(self):
        ...
        assert self.repository_data_path.exists(), (
            f"Repository data not found at {self.repository_data_path}"
        )
        ...
        assert "SQLAlchemy" in frameworks, (
            f"SQLAlchemy not detected in frameworks: {frameworks}"
        )
        ...
        assert len(models) >= 2, (
            f"Expected at least 2 SQLModel tables, found {len(models)}"
        )
        assert "Repository" in models, f"Repository not found in models: {models}"
        assert "CodebaseConfig" in models, (
            f"CodebaseConfig not found in models: {models}"
        )

        # Test relationship patterns
        assert self._has_relationship_patterns(signature), (
            "Relationship patterns not found"
        )
    ...
    def test_complex_framework_combination_from_real_main_py(self):
        ...
        assert self._has_dependency_injection_patterns(signature), (
            "Dependency injection patterns not found"
        )
    ...
    def test_performance_comparison_with_real_main_py(self):
        ...
        assert "FastAPI" in common_frameworks, (
            "FastAPI should be detected by both methods"
        )
        ...
        print(
            f"Structural Signature: {structural_time:.4f}s, detected: {frameworks_structural}"
        )
        ...
        assert len(frameworks_structural) <= len(frameworks_string), (
            "Structural signature should detect fewer frameworks (definitions only, not imports)"
        )
    ...
    def test_framework_detection_across_multiple_real_files(self):
        ...
        files_to_test = [
            ...
            (
                self.repository_data_path,
                ["SQLAlchemy"],
            ),  # Only SQLAlchemy expected since we migrated to SQLBase
        ]
        ...
        for file_path, expected_frameworks in files_to_test:
            ...
            for framework in expected_frameworks:
                assert framework in detected_frameworks, (
                    f"{framework} not detected in {file_path.name}"
                )
    ...
    def _has_fastapi_patterns(self, signature: PythonStructuralSignature) -> bool:
        ...
        for func in signature.functions:
            if "@app." in func.signature and any(
                method in func.signature for method in ["get", "post", "put", "delete"]
            ):
                return True
    ...
    def _has_sqlalchemy_patterns(self, signature: PythonStructuralSignature) -> bool:
        ...
        if (
            "sqlalchemy" in all_text.lower()
            or "sqlmodel" in all_text.lower()
            or "sqlbase" in all_text.lower()
        ):
            return True
    ...
    def _has_pydantic_field_patterns(
        self, signature: PythonStructuralSignature
    ) -> bool:
    ...
    def _has_dependency_injection_patterns(
        self, signature: PythonStructuralSignature
    ) -> bool:
    ...
    def _has_activity_execution_patterns(
        self, signature: PythonStructuralSignature
    ) -> bool:
    ...
    def _has_child_workflow_patterns(
        self, signature: PythonStructuralSignature
    ) -> bool:
    ...
    def _has_temporal_client_patterns(
        self, signature: PythonStructuralSignature
    ) -> bool:
    ...
    def _has_complex_field_types(self, signature: PythonStructuralSignature) -> bool:
        ...
        for cls in signature.classes:
            for var in cls.vars:
                if (
                    "Dict[" in var.signature
                    or "List[" in var.signature
                    or "Optional[" in var.signature
                ):
                    return True
        return False
    ...
    def _extract_temporal_workflows(
        self, signature: PythonStructuralSignature
    ) -> List[str]:
    ...
    def _extract_sqlmodel_tables(
        ...
        for cls in signature.classes:
            ...
            is_table = (
                "SQLModel" in cls.signature and "table=True" in cls.signature
            ) or "SQLBase" in cls.signature
            if is_table:
                # Extract class name from signature
                if "class " in cls.signature:
                    class_name = (
    ...
    def _extract_pydantic_validators(
        self, signature: PythonStructuralSignature
    ) -> List[str]:
            ...
            for method in cls.methods:
                if "@field_validator" in method.signature:
                    ...
                    if "def " in method.signature:
                        method_name = (
                            method.signature.split("def ")[1].split("(")[0].strip()
                        )
