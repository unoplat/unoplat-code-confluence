...
def test_types_query_interface_ranges(
    ...
    for name, (expected_start, expected_end, expected_signature) in expected_interfaces.items():
        assert name in interface_ranges, f"Missing interface capture for {name}"
        start_line, end_line, signature = interface_ranges[name]
        assert start_line == expected_start, f"{name} start line mismatch (expected {expected_start}, got {start_line})"
        assert end_line == expected_end, f"{name} end line mismatch (expected {expected_end}, got {end_line})"
        assert signature == expected_signature, f"{name} signature mismatch (expected '{expected_signature}', got '{signature}')"
...
def test_types_query_type_alias_ranges(
    ...
    expected_aliases = {
        "FilterItemSchema": (320, 320, "export type FilterItemSchema = z.infer<typeof filterItemSchema>;"),
        "ExtendedColumnSort": (322, 326, "export type ExtendedColumnSort<TData> = {"),
        "ModelProviderDefinitions": (338, 340, "export type ModelProviderDefinitions = ReadonlyArray<"),
    }

    for name, (expected_start, expected_end, expected_signature) in expected_aliases.items():
