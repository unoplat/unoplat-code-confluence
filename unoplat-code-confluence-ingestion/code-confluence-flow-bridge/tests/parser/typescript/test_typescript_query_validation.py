    ...
    for query_name, filename in TYPESCRIPT_QUERY_FILES.items():
        ...
        if query_name in OPTIONAL_QUERIES:
            ...
            assert query_path.exists(), (
                f"Required query file {filename} not found at {query_path}"
            )
...
def test_query_file_compiles(
    ...
    try:
        ...
        assert compiled_query is not None, (
            f"Query compilation returned None for {filename}"
        )
...
def test_imports_query_basic_functionality(
    ...
    # Execute query
    matches: list[tuple[int, dict[str, list[tree_sitter.Node]]]] = cursor.matches(
        tree.root_node
    )
...
def test_functions_query_basic_functionality(
    ...
    for _match_id, captures_dict in matches:
        for capture_name, nodes in captures_dict.items():
            ...
            component_counts[capture_name] = component_counts.get(
                capture_name, 0
            ) + len(nodes)
...
def test_classes_query_component_captures(
    ...
    print(
        f"✓ classes.scm captured {total_components} component captures across {num_classes} classes"
    )
...
def test_methods_query_component_captures(
    ...
    total_components = sum(component_counts.values())
    method_count = (
        component_counts.get("method_name", 0)
    ...
    print(
        f"✓ methods.scm captured {total_components} component captures across {method_count} methods"
    )
...
def test_exports_query_functionality(
    ...
    print(
        f"✓ exports.scm validated against AST sample export patterns ({len(matches)} matches)"
    )
...
def test_methods_query_functionality(
    ...
    print(
        f"✓ methods.scm validated against AST sample method patterns ({len(matches)} matches)"
    )
...
def test_variables_query_functionality(
    ...
    assert (
        "const_let_variable_name" in capture_names
        or "var_variable_name" in capture_names
        or "property_name" in capture_names
    ), "Should capture variable names"

    print(
        f"✓ variables.scm validated against AST sample variable patterns ({len(matches)} matches)"
    )
...
def test_enums_query_functionality(
    ...
    print(
        f"✓ enums.scm validated against AST sample enum patterns ({len(matches)} matches)"
    )
...
def test_namespaces_query_functionality(
    ...
    assert len(matches) > 0, (
        "namespaces.scm should capture namespace/module declarations"
    )
    assert (
        "namespace_name" in capture_names
        or "module_name" in capture_names
        or "module_declaration" in capture_names
    ), "Should capture namespace or module names"

    print(
        f"✓ namespaces.scm validated against AST sample namespace patterns ({len(matches)} matches)"
    )
...
def test_query_capture_names_documented(typescript_query_dir: Path) -> None:
    ...
    for query_name, filename in TYPESCRIPT_QUERY_FILES.items():
        ...
        assert has_comment or has_capture, (
            f"{filename} should have comments or captures documented"
        )


def test_all_queries_summary(
    typescript_language: tree_sitter.Language, typescript_query_dir: Path
) -> None:
    ...
    total_required = len(
        [q for q in TYPESCRIPT_QUERY_FILES.keys() if q not in OPTIONAL_QUERIES]
    )
