...
class TestFrameworkDefinitionsIngestion:
    ...
    async def test_real_framework_data_parsing_accuracy(self):
        ...
        assert len(fastapi_features) == 3, f"Expected 3 FastAPI features, got {len(fastapi_features)}"
        
        http_endpoint = next((f for f in fastapi_features if f.feature_key == "http_endpoint"), None)
        assert http_endpoint is not None, "FastAPI http_endpoint feature not found"
        assert http_endpoint.description == "HTTP verb decorator that registers a route handler"
        ...
        assert http_endpoint.startpoint is True, "http_endpoint should be marked as startpoint"
        ...
        http_endpoint_paths = [ap for ap in absolute_paths 
                             if ap.library == "fastapi" and ap.feature_key == "http_endpoint"]
        assert len(http_endpoint_paths) == 2, f"Expected 2 paths for http_endpoint, got {len(http_endpoint_paths)}"
        ...
        for feature in features:
            ...
            assert feature.concept in ["AnnotationLike", "CallExpression", "Inheritance"]
