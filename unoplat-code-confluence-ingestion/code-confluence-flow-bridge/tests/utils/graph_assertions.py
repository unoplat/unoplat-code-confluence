...
def count_nodes_by_label_and_qualifier(
    neo4j_client, label: str, qualifier_prefix: str
) -> int:
    """
...
def count_relationships_by_type(neo4j_client, relationship_type: str) -> int:
    ...
    """
    results, _ = neo4j_client.cypher_query(
        f"MATCH ()-[r:{relationship_type}]-() RETURN count(r) AS c"
    )
    return results[0][0] if results else 0
...
def get_relationship_type_counts(neo4j_client) -> Dict[str, int]:
    ...
    """
    results, _ = neo4j_client.cypher_query(
        "MATCH ()-[r]-() RETURN type(r) AS rel_type, count(r) AS c"
    )
    counts: Dict[str, int] = {}
    for row in results:
...
def find_nodes_related_to_repository(
    neo4j_client, repo_qualified_name: str
) -> Dict[str, List[str]]:
    """
...
def verify_complete_repository_deletion(
    neo4j_client, repo_qualified_name: str
) -> Dict[str, Any]:
    """
    ...
    for label in node_labels_to_check:
        ...
        if count > 0:
            verification_results["issues"].append(
                f"Found {count} residual {label} nodes"
            )
...
def capture_repository_state_snapshot(
    neo4j_client, repo_qualified_name: str
) -> Dict[str, Any]:
    """
    ...
    for label in graph_model_labels:
        if label == "CodeConfluenceGitRepository":
            ...
            snapshot["node_counts"][label] = (
                1 if repo_exists(neo4j_client, repo_qualified_name) else 0
            )
        else:
...
def assert_deletion_stats_accuracy(
    deletion_stats: Dict[str, int], pre_snapshot: Dict[str, Any]
) -> None:
    """
    ...
    if files_count == 0:
        mismatches.append(
            "Sanity check failed: No files existed after ingestion to delete"
        )
...
def assert_repository_completely_deleted(
    neo4j_client, repo_qualified_name: str
) -> None:
    """
    ...
    verification_results = verify_complete_repository_deletion(
        neo4j_client, repo_qualified_name
    )

    if verification_results["issues"]:
        error_msg = (
            f"❌ REPOSITORY DELETION VERIFICATION FAILED for {repo_qualified_name}:\n"
        )
        error_msg += "\n".join(
            f"  - {issue}" for issue in verification_results["issues"]
        )
        error_msg += (
