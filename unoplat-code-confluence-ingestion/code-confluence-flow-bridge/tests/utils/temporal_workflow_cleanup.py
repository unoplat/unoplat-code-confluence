...
async def terminate_all_running_workflows(temporal_address: str, namespace: str = "default") -> int:
    """
    ...
    try:
        ...
        async for workflow in client.list_workflows(query):
            try:
                ...
                await handle.terminate(reason="Test cleanup - forceful termination of running workflows")
                terminated_count += 1
                logger.debug(f"TERMINATED workflow: {workflow.id} (run: {workflow.run_id})")
    ...
    if failed_terminations:
        logger.warning(f"Failed to terminate {len(failed_terminations)} workflows: {failed_terminations}")
    
    return terminated_count


async def terminate_workflows_by_type(temporal_address: str, workflow_type: str, namespace: str = "default") -> int:
    """
    ...
    async for workflow in client.list_workflows(query):
        try:
            ...
            await handle.terminate(reason=f"Test cleanup - terminating {workflow_type} workflows")
            terminated_count += 1
            logger.debug(f"TERMINATED {workflow_type} workflow: {workflow.id}")
            
        except Exception as e:
            logger.warning(f"Failed to terminate {workflow_type} workflow {workflow.id}: {e}")
            continue
