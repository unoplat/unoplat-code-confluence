                            ...
                            try:
                                if IsClassName.is_python_class_name(usage.original_name):
                                    ...
                                    try:
                                        ...
                                        logger.error("Missing source module in dictionary: {}\n" "Available modules: {}\n" "Current node: {}", imp.source, list(qualified_dict.keys()), node.node_name)
                                        continue

                                    if procedural_dependent_node.node_name not in procedural_nodes:
                                        logger.debug("Found procedural dependency: {} -> {}", node.node_name, procedural_dependent_node.node_name)
                                ...
                                logger.error("Error processing import usage: {}\n" "Import source: {}\n" "Error: {}", usage.original_name, imp.source, str(inner_e))
                                continue

            logger.debug("Completed processing dependencies for node: {}\n" "Found {} class dependencies and {} procedural dependencies", node.node_name, len(node.dependent_internal_classes) - len(procedural_nodes), len(procedural_nodes))
