...
export function agentMdOutputToMarkdown(agent: AgentMdOutput, options?: AgentMdToMarkdownOptions): string {
  ...
  if (agent?.project_configuration?.config_files?.length) {
    entries.push({ ul: agent.project_configuration.config_files.map((f) => `${f.path} — ${f.purpose}`) });
  } else {
  ...
  const byKind: Record<string, { kind: string; command: string; description?: string; config_files: string[] }[]> = {};
  ...
  for (const kind of Object.keys(byKind)) {
    ...
    entries.push({ ol: byKind[kind].map((c) => `${c.command}${c.description ? ` — ${c.description}` : ''}`) });
  }
  ...
  if (agent?.business_logic?.data_models?.length) {
    ...
    entries.push({ ul: agent.business_logic.data_models.map((m) => `${m.path}${m.responsibility ? ` — ${m.responsibility}` : ''}`) });
  }

  return json2md(entries as any);
}

export function codebasesToMarkdown(codebases: Record<string, AgentMdOutput>, options?: AgentMdToMarkdownOptions): string {
  const title: string = options?.title ?? 'Agents Documentation';
  const entries: unknown[] = [];
