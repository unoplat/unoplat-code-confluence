import { ApiErrorReport, UiErrorReport, IssueType, IssueTracking } from "../types";
...
export function apiToUiErrorReport(
  ...
  const issueTracking = apiError.metadata?.issue_tracking as IssueTracking | undefined;
  
  // Handle stack_trace which could be string | null | undefined
  // Convert null to undefined to satisfy error_traceback type
  const errorTraceback = apiError.stack_trace === null ? undefined : apiError.stack_trace;
  ...
  return {
    ...
    ...(context.workflow_run_id && { workflow_run_id: context.workflow_run_id }),
...
export function formatErrorReportContent(errorReport: UiErrorReport): string {
  const title = errorReport.error_type === 'CODEBASE' 
