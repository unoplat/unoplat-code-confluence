...
function validateGitRepositoryFiles(files: FileList): boolean {
  console.log(
    "[Git Validation] validateGitRepositoryFiles called – total files:",
    files.length,
  );
  ...
  console.log(
    "[Git Validation] No .git directory found in any file paths – validation failed",
  );
  return false;
}
...
export function validateGitRepositorySync(input: DirectoryInput): boolean {
  console.log(
    "[Git Validation] validateGitRepositorySync called with input:",
    input,
  );
  ...
  try {
    ...
    if (input instanceof FileList) {
      console.log(
        "[Git Validation] Input is FileList, using synchronous webkitdirectory validation",
      );
      ...
      console.log(
        "[Git Validation] FileSystemDirectoryHandle not supported in sync mode, returning false",
      );
      return false;
    }
  } catch (error) {
