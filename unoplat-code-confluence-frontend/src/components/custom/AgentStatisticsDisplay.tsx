...
import type { WorkflowStatistics, UsageStatistics } from "@/features/repository-agent-snapshots/schema";
...
function UsageStatsGrid({ stats }: { stats: UsageStatistics }): React.ReactElement {
  return (
    <div className="grid grid-cols-2 gap-3 text-sm">
      ...
      <div>
        ...
        <div className="font-medium">{formatNumber(stats.cache_read_tokens)}</div>
      </div>
      <div>
        <div className="text-muted-foreground text-xs">Cache Write</div>
        <div className="font-medium">{formatNumber(stats.cache_write_tokens)}</div>
      </div>
      <div>
      ...
      <div>
        ...
        <div className="font-medium">{formatCost(stats.estimated_cost_usd)}</div>
      </div>
    </div>
  );
}
...
export function AgentStatisticsDisplay({
  ...
  return (
    <div className="space-y-4">
      <Card className="p-4">
        <h3 className="mb-3 text-sm font-semibold">Overall Workflow Statistics</h3>
      ...
      {hasCodebaseBreakdown && (
        <Card className="p-4">
          ...
          <div className="space-y-2">
            {Object.entries(statistics.by_codebase).map(([codebaseName, stats]) => (
              <Collapsible key={codebaseName}>
                <CollapsibleTrigger className="flex w-full items-center justify-between rounded-md border p-2 text-sm hover:bg-accent">
