...
export const RepositoryDataTable = forwardRef<RepositoryDataTableRef, { tokenStatus: boolean; onSelectionChange?: (selectedRows: string[]) => void; }>(
  ({ tokenStatus, onSelectionChange }, ref) => {
    ...
    const searchParams = OnboardingRoute.useSearch();
    console.log('[RepositoryDataTable] Parsed search params:', searchParams);

    // Parse the current page number from the URL; default to 1 if not specified.
    const page = useMemo(() => Number(searchParams.pageIndex || 1), [searchParams.pageIndex]);
    // Parse the current items-per-page count; default to 10 if not specified.
    const perPage = useMemo(() => Number(searchParams.perPage || 10), [searchParams.perPage]);
    console.log('[RepositoryDataTable] Current pagination:', { page, perPage });
    ...
    const { data: repoData, isLoading, isFetching, status } = useQuery<PaginatedResponse<GitHubRepoSummary>>({
      queryKey: ['repositories', page, perPage, cursors[page - 1]],
      ...
      queryFn: () => {
        ...
        return fetchGitHubRepositories(page, perPage, undefined, cursors[page - 1]);
      },
    ...
    useEffect(() => {
      ...
      if (repoData?.next_cursor) {
        ...
        console.log('[RepositoryDataTable] Updating cursor for page', page);
        setCursors(prev => {
          ...
          if (page >= newCursors.length) {
            ...
            newCursors[page] = repoData.next_cursor;
          }
          return newCursors;
        });
      }
    ...
    const { table } = useDataTableWithRouter({
      ...
      pageCount: repoData?.has_next ? -1 : page,
    ...
    }, [table.getState().pagination, page, perPage]);
