...
import { repositoryAgentSnapshotRowSchema, type RepositoryAgentSnapshotRow } from './schema';
...
const createScopeKey = ({ owner, repository }: RepositoryAgentSnapshotScope): string =>
  `${owner}/${repository}`;

function createCollectionForScope({ owner, repository }: RepositoryAgentSnapshotScope) {
  return createCollection(
    electricCollectionOptions({
      id: `repository-agent-snapshots-${owner}-${repository}`,
      schema: repositoryAgentSnapshotRowSchema,
      getKey: (row: RepositoryAgentSnapshotRow) => `${row.repository_owner_name}/${row.repository_name}`,
      shapeOptions: {
...
export type RepositoryAgentSnapshotCollection = ReturnType<typeof createCollectionForScope>;

const snapshotCollections = new Map<string, RepositoryAgentSnapshotCollection>();
