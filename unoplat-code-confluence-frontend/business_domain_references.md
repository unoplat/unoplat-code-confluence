...
## Domain Summary
This frontend supports a developer platform for onboarding source repositories, ingesting and analyzing their codebases through AI-agent workflow runs, and presenting generated engineering workflows, dependency guides, business-logic summaries, interface findings, progress, events, and usage statistics. It also manages repository-provider credentials, model and tool configurations including OAuth, operational status and issue reporting, and user feedback on both the application and individual agent outputs.
