...
class MockSSEService:
    ...
    def __init__(
        ...
        self.result_file_path = Path(result_file_path) if result_file_path else self.events_file_path.parent / "agent-md-result.json"
    ...
    def _load_fixture(self) -> None:
        try:
            ...
            logger.error("Mock events file {} could not be parsed: {}", self.events_file_path, error)
            return
        ...
        try:
            ...
            logger.warning("Mock agent MD result file not found at {}", self.result_file_path)
            self._final_payload = {
                "repository": self.repository_name or "mock/repository",
                "codebases": {},
            }
        except json.JSONDecodeError as error:
            logger.warning("Mock agent MD result file {} could not be parsed: {}", self.result_file_path, error)
            self._final_payload = {
                "repository": self.repository_name or "mock/repository",
                "codebases": {},
            }
