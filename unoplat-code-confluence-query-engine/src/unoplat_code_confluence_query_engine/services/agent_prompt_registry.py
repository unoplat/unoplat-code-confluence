...
class AgentPromptRegistry(PromptProviderProtocol):
    ...
    def _business_logic_domain_prompt(
        ...
        header = (
            f"Identify the dominant business logic domain for repository {repository_qualified_name}. "
            f"Focus ONLY on the codebase '{codebase_name}' located at {codebase_path} written in {codebase_language}. "
            f"Return only a plain text description (2-4 sentences) of the business domain.\n\n"
        )

        instructions = (
            "Workflow:\n"
            "1) Call get_data_model_files to retrieve all data model file paths with their (start_line, end_line) spans\n"
            "2) Build a coverage checklist from every (file, model) pair\n"
            "3) Process UNTIL the checklist is empty:\n"
            "   - For each remaining pair, call read_file_content with the provided span\n"
            "   - Extract the model's intent and purpose\n"
            "   - Mark as processed and continue\n"
            "   - If tool calls fail after 2 retries, note and continue\n"
            "4) Synthesize the dominant business domain from ALL inspected models\n"
            "5) Return ONLY a plain text description (2-4 sentences) covering:\n"
            "   - Dominant domain nouns (users, orders, products, etc.)\n"
            "   - Core business processes or capabilities\n"
            "   - Primary domain purpose\n\n"
            "Do NOT return JSON. Do NOT return file lists. Return ONLY the domain description as plain text.\n"
        )
