...
class InboundKind(str, Enum):
    ...
    DB_CHANGEFEED = "db_changefeed_consumer"    # Debezium/binlog/Mongo CS/DynamoDB streams
    OBJECT_STORAGE_EVENT = "object_storage_event" # S3/GCS/Azure Blob notifications

    MQTT_IOT_ENDPOINT = "mqtt_iot_endpoint"     # MQTT broker/endpoint (AMQP1.0/CoAP -> map here if used)
...
class CommandSpec(BaseModel):
    ...
    config_files: List[str] = Field(
        default_factory=list, description="Relevant config files with relative path to the codebase root"
    )
...
class CodebaseMetadataOutput(BaseModel):
    ...
    architectural_pattern: Optional[List[BackendArchitecturalType]] = Field(None, description="Type of architectural pattern if current codebase is of type backend")
    frontend_rendering_strategy: Optional[FrontendRenderingStrategyType] = Field(None, description="Type of frontend rendering strategy if current codebase if of type frontend")
...
class InboundConstruct(BaseModel):
    ...
    library: Optional[str] = Field(..., description="Library/Framework used for the inbound construct if applicable.")
    match_pattern: Dict[str, List[str]] = Field(..., description="relative w.r.t codebase path - File path as key with corresponding list of match patterns where the inbound construct is used")
    model_config = ConfigDict(extra="forbid")
    
class OutboundConstruct(BaseModel):
    kind: OutboundKind = Field(..., description="Kind of outbound construct")
    library: Optional[str] = Field(..., description="Library/Framework used for the outbound construct if applicable.")
    match_pattern: Dict[str, List[str]] = Field(..., description="relative w.r.t codebase path - File path as key with corresponding list of match patterns where the outbound construct is used")
    model_config = ConfigDict(extra="forbid")
...
class AgentMdOutput(BaseModel):
    ...
    project_configuration: ProjectConfiguration = Field(..., description="Project configuration")
    #TODO: remove optional when ready
    frameworks_and_libraries: Optional[List[FrameworkLibraryOutput]] = Field(
        default=None, description="frameworks and libraries used and the overview of the functionality they provide."
    )
