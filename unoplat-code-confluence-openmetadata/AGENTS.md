...
### Install
- `uv sync --group dev --group test` — run from the repository root using `pyproject.toml`.
...
### Test
- `uv run --group test pytest -v tests/` — run from the repository root using `pyproject.toml`.

### Lint
- `uv run --group dev ruff check src/` — run from the repository root using `pyproject.toml`.

### Type Check
- `uv run --group dev pyrefly check src/` — run from the repository root using `pyproject.toml`.
