...
- **Package:** engine
  ...
  - **Objective:** <p>To extend the functionality of existing classes and models to generate text based on provided prompts and prompts from external sources, utilizing language models and cached responses when necessary.</p>

  - **Summary:** <p>The "ChatGemini" class extends the functionality of the "EngineLM" and "CachedEngine" classes to generate text based on the provided "content" parameter. The "ChatAnthropic" class builds upon this functionality by utilizing the Anthropic language model to generate responses based on a given prompt. The "ChatTogether" class extends the "ChatOpenAI" class and initializes the "ChatExternalClient" class with the provided parameters. The "ChatCohere" class generates a response using a chatbot model, checking for a cached response and sending a chat request if necessary. The objective of the "EngineLM" class is to generate output based on a given prompt and optional system prompt, utilizing additional keyword arguments</p>

### Class Summaries

- **ChatGemini**

  - **Objective:** <p>Initialize the system prompt, model string, and necessary variables for the ChatGemini class.</p>

  - **Summary:** <p>The ChatGemini class is responsible for initializing the system prompt, model string, and other necessary variables. It extends the EngineLM and CachedEngine classes and imports modules such as google.generativeai, os, platformdirs, and tenacity. The class implements the __call__ method, which sets the model string and system prompt as attributes of the class.</p>
...
- **ChatOpenAI**
  ...
  - **Objective:** <p>The objective of the "ChatOpenAI" class is to generate text based on the provided "content" parameter, extending the functionality of the "EngineLM" and "CachedEngine" classes.</p>

  - **Summary:** <p>The "ChatOpenAI" class initializes an instance with the provided parameters and sets the "system_prompt" attribute. It imports necessary modules and classes, including "OpenAI", "os", "json", "base64", "platformdirs", "tenacity", "typing", ".base", and ".engine_utils". The objective of the "generate" function is to generate text based on the provided "content" parameter, which can be a string or a list of strings or bytes. It determines the appropriate method to call based on the type of "content" and utilizes the "ChatOpenAI" class to extend the functionality of the "EngineLM" and "CachedEngine" classes. The "_generate_from_single_prompt"</p>
...
- **ChatAnthropic**
  ...
  - **Objective:** <p>The objective of the "ChatAnthropic" class is to generate responses based on a given prompt using the Anthropic language model, while also providing methods for formatting content and handling multiple inputs.</p>

  - **Summary:** <p>The "ChatAnthropic" class is responsible for initializing an instance by setting the necessary parameters and importing required modules. It extends the "EngineLM" and "CachedEngine" classes and imports modules such as "anthropic", "os", "platformdirs", "tenacity", "base64", "json", "typing", ".base", and ".engine_utils". The class includes the "__call__" method which generates a response based on the given prompt using the Anthropic language model. The class also includes the "_format_content" method which formats a list of strings and bytes into a list of dictionaries representing images. The class includes the "_generate_from_multiple_input" method which takes in multiple inputs and optional parameters, checks</p>
...
- **ChatExternalClient**
  ...
  - **Objective:** <p>To extend the "ChatOpenAI" class and initialize the "ChatExternalClient" class with the provided parameters.</p>

  - **Summary:** <p>The "ChatExternalClient" class is initialized with the provided parameters and extends the "ChatOpenAI" class.</p>
...
- **ChatTogether**
  ...
  - **Objective:** <p>The objective of the "ChatTogether" class is to generate a response based on a given prompt and system prompt using the OpenAI chat completions API, while also checking for and utilizing cached responses when available.</p>

  - **Summary:** <p>The "ChatTogether" class is initialized with a model string, system prompt, and a root variable. It extends the "EngineLM" and "CachedEngine" classes. The class imports modules such as "together", "os", "platformdirs", and "tenacity". The "generate" function generates a response based on the given prompt and system prompt using the OpenAI chat completions API. It checks for a cached response and returns it if available, otherwise it generates a new response and saves it in the cache. The "__call__" method in the "ChatTogether" class extends "EngineLM" and "CachedEngine" to take in a prompt and additional keyword arguments, without specifying a return type.</p>
...
- **ChatCohere**
  ...
  - **Objective:** <p>The objective of the "ChatCohere" class is to generate a response using a chatbot model, checking for a cached response and sending a chat request if necessary.</p>

  - **Summary:** <p>The "ChatCohere" class is a subclass of both the "EngineLM" and "CachedEngine" classes. It is responsible for generating a response using a chatbot model. The class checks if a cached response is available based on the system and input prompts, and returns it if found. If no cached response is available, it sends a chat request to the ChatCohere model, utilizing the tenacity library for retrying the request.</p>
...
- **EngineLM**
  ...
  - **Objective:** <p>The objective of the "EngineLM" class is to generate output based on a given prompt and optional system prompt, utilizing additional keyword arguments and local variables to enhance the generation process.</p>

  - **Summary:** <p>The "EngineLM" class is an extension of the "ABC" class and is responsible for generating output based on a given prompt and optional system prompt. It utilizes additional keyword arguments and local variables to enhance the generation process. The class imports necessary modules such as "hashlib", "diskcache" (imported as "dc"), and "abc" (imported as "ABC" and "abstractmethod").</p>
...
- **CachedEngine**
  ...
  - **Objective:** <p>The objective of the CachedEngine class is to provide a caching mechanism for prompt and response data by calculating hash values, checking cache existence, and saving data into a cache file.</p>

  - **Summary:** <p>The CachedEngine class initializes a class instance by setting the "cache_path" attribute. It includes the functions "_hash_prompt", "_check_cache", and "_save_cache" for calculating the hash value of the input prompt, checking if a prompt exists in the cache, and saving prompt and response data into a cache file, respectively. The class also has the function "__setstate__" which updates the cache by restoring it after unpickling and assigns it to the variable "self.cache".</p>
